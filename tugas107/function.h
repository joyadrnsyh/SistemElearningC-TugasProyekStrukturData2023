...
public:
    ...
    // Sorting based on end date (bubble sort)
    void sortTasksByEndDate() {
        for (int i = 0; i <= top; ++i) {
            for (int j = 0; j < top - i; ++j) {
                if (tasks[j].endDate > tasks[j + 1].endDate) {
    ...
    // Sorting based on start date (bubble sort)
    void sortTasksByStartDate() {
        for (int i = 0; i <= top; ++i) {
            for (int j = 0; j < top - i; ++j) {
                if (tasks[j].startDate > tasks[j + 1].startDate) {
                    swap(tasks[j], tasks[j + 1]);
                }
            }
        }
      ...
      string fileFilter;
        switch (choice) {
            case 1:
                ...
                taskStack.sortTasksByEndDate(); // Sort after adding a new task
                break;
            case 2:
                int viewChoice;
                cout << "Menu Lihat Tugas:\n1. Tampilkan semua tugas\n2. Tampilkan berdasarkan Tipe File\n3. Urutkan berdasarkan Tanggal Selesai\n4. Urutkan berdasarkan Tanggal Mulai\nPilih: ";
                cin >> viewChoice;
                ...
                switch (viewChoice) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        taskStack.sortTasksByEndDate();
                        taskStack.displayTasks();
                        break;
                    case 4:
                        taskStack.sortTasksByStartDate();
                        taskStack.displayTasks();
                        break;
                    default:
